  ...
  return (
    <div style={{ fontFamily: 'system-ui, -apple-system, sans-serif', color: DARK, overflowX: 'hidden' }}>
      ...
      <style>{`
        ...
        /* ── Button hovers ── */
        .btn-green   { transition: background 0.18s ease, transform 0.18s ease, box-shadow 0.18s ease !important; }
        .btn-green:hover { background: #15803D !important; transform: translateY(-2px); box-shadow: 0 6px 18px rgba(22,163,74,0.35); }
        ...
        .btn-report:hover { background: #FEE2E2 !important; border-color: #FCA5A5 !important; }
      ...
      <nav className="nav-in" style={{
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <button onClick={() => navigate('/report')} className="btn-report"
            style={{ padding: '8px 20px', background: '#FEF2F2', border: '1.5px solid #FECACA', borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: 'pointer', color: '#DC2626' }}>
            Report a Concern
          </button>
          ...
          <button onClick={() => navigate('/login')} className="btn-green"
            style={{ padding: '8px 20px', background: GREEN, border: 'none', borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: 'pointer', color: '#fff' }}>
            Staff Access
          </button>
        </div>
      </nav>
      ...
      <section style={{
        ...
        <div style={{ position: 'absolute', top: '20%', left: '38%', width: 500, height: 500, borderRadius: '50%', background: 'rgba(22,163,74,0.08)', filter: 'blur(90px)', pointerEvents: 'none' }} />

        <div style={{ position: 'relative', zIndex: 1, maxWidth: 680, padding: '0 80px' }}>
          <div className="hero-badge" style={{ display: 'inline-block', fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', color: '#86EFAC', textTransform: 'uppercase', marginBottom: 20, background: 'rgba(22,163,74,0.15)', padding: '5px 14px', borderRadius: 20, border: '1px solid rgba(134,239,172,0.3)' }}>
          ...
          <h1 className="hero-h1" style={{ fontSize: 60, fontWeight: 900, color: '#fff', margin: '0 0 24px', lineHeight: 1.07, letterSpacing: '-2px' }}>
            Connecting those<br />
            who <span style={{ color: '#4ADE80' }}>protect</span><br />
            children in need.
          </h1>
          ...
          <div className="hero-btns" style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <button onClick={() => navigate('/login')} className="btn-green"
              style={{ padding: '14px 32px', background: GREEN, color: '#fff', border: 'none', borderRadius: 7, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
              Get Started →
            </button>
      ...
      ══════════════════════════════════════ */}
      <section style={{ background: '#FFF5F5', borderBottom: '1px solid #FECACA', padding: '22px 80px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#991B1B' }}>Witnessed a child at risk?</div>
            <div style={{ fontSize: 13, color: '#B91C1C', marginTop: 2 }}>You can report it anonymously. Every report is reviewed by our team.</div>
          </div>
        </div>
        <button onClick={() => navigate('/report')}
          style={{ padding: '11px 28px', background: '#DC2626', color: '#fff', border: 'none', borderRadius: 7, fontSize: 14, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
          Report a Concern →
        </button>
      </section>
      ...
      <section style={{ padding: '96px 80px', background: '#fff' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 72, alignItems: 'center' }}>
          ...
          <div>
            <p data-animate style={{ fontSize: 12, fontWeight: 700, color: GREEN, letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 12px' }}>Who it's for</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[
                ...
                { icon: '🤝', title: 'NGO Organisations', accent: GREEN,      bg: '#F0FDF4', desc: 'Receive assigned cases, review documents, and post progress updates.', delay: '4' },
                { icon: '👷', title: 'Social Workers',    accent: '#EA580C',  bg: '#FFF7ED', desc: 'Register children, open cases, and upload supporting documents from the field.', delay: '5' },
      ...
      <section id="how-it-works" style={{
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(110deg, rgba(4,40,20,0.93) 0%, rgba(4,40,20,0.88) 50%, rgba(4,40,20,0.70) 100%)' }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
          {/* Left */}
          <div>
            <p data-animate data-animate="fade" style={{ fontSize: 11, fontWeight: 700, color: '#6EE7B7', letterSpacing: '0.14em', textTransform: 'uppercase', margin: '0 0 14px' }}>The Process</p>
            ...
            <p data-animate data-delay="2" style={{ fontSize: 15, color: '#A7F3D0', lineHeight: 1.75, margin: '0 0 36px' }}>
            ...
            {[
              ...
              <div key={n} data-animate data-delay={String(i + 1)}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: GREEN, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, flexShrink: 0 }}>{n}</div>
                ...
                <div style={{ paddingTop: 7 }}>
                  ...
                  <div style={{ fontSize: 13, color: '#A7F3D0', lineHeight: 1.65 }}>{desc}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ══════════════════════════════════════
          CTA BANNER
      ══════════════════════════════════════ */}
      <section style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, #14532D 0%, #166534 100%)', padding: '90px 80px', textAlign: 'center' }}>
        ...
        <div data-animate="scale-in" style={{ position: 'relative', zIndex: 1 }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: '#6EE7B7', letterSpacing: '0.14em', textTransform: 'uppercase', margin: '0 0 14px' }}>Ready to help?</p>
          ...
          <p style={{ color: '#A7F3D0', fontSize: 16, margin: '0 0 36px' }}>
            Log in with your credentials to access your dashboard.
          </p>
          <button onClick={() => navigate('/login')} className="btn-white"
            style={{ padding: '14px 36px', background: '#fff', color: '#14532D', border: 'none', borderRadius: 7, fontSize: 15, fontWeight: 800, cursor: 'pointer' }}>
